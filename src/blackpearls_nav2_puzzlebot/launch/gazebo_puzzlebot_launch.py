...
def generate_launch_description():
    ...
    models = get_package_share_directory('blackpearls_nav2_puzzlebot')
...
# Node that spawns the robot in Gazebo from its robot_description
    ...
    start_gazebo_ros_bridge_jetson_lidar_ed = Node(
        ...
        namespace=prefix_name,
        parameters=[{
            'config_file': os.path.join(models, 'config', 'puzzlebot_jetson_lidar_ed.yaml'),}],
        output='screen'
    )
