...
class FramePublisherClass(Node):
    def __init__(self):
        ...
        self.r = 0.052  # radio de la rueda
        ...
        self.ctrlJoints.position = [0.0, 0.0]
        self.ctrlJoints.velocity = [0.0, 0.0]
        self.ctrlJoints.effort = [0.0, 0.0]
        ...
        # Variables para publicar velocidades angulares
        self.w_r = Float32()
        self.w_l = Float32()
    ...
    def vel_callback(self, msg):
        """Callback para recibir velocidades lineales y angulares del tópico 'cmd_vel'."""
        self.robot_v = msg.linear.x
        self.robot_w = msg.angular.z

    def timer_callback(self):
        """Callback periódico para actualizar y publicar transformaciones y estados."""
        if self.begin:
        ...
        dt = float(self.current_time - self.previous_time) / 1e9
        ...
        # Actualización de la posición y orientación del robot
        self.robot_x += self.robot_v * np.cos(self.robot_theta) * dt
        self.robot_y += self.robot_v * np.sin(self.robot_theta) * dt
        self.robot_theta += self.robot_w * dt

        # Actualización de los ángulos de las ruedas
        self.r_theta += wr * dt
        self.l_theta += wl * dt
        ...
        # Publicar velocidades angulares de las ruedas
        self.w_r.data = wr
        self.w_l.data = wl
